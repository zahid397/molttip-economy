...
export const TokenBalance: React.FC<TokenBalanceProps> = ({
  ...
  const deltaPercent = previousBalance && previousBalance > 0
  ...
  return (
    <div className="card card-glow text-center relative overflow-hidden">
      ...
      {/* Main balance */}
      <p className={cn(
        'font-display font-bold text-glow-cyan leading-none',
        valueSize
      )}>
      ...
      {TrendIcon && delta !== null && (
        <div className={cn(
          ...
          {deltaPercent && (
            <span className="opacity-60">({delta > 0 ? '+' : ''}{deltaPercent}%)</span>
          )}
        </div>
      )}
